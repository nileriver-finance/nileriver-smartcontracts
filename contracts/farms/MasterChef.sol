    ...
    modifier checkHalving() {
        if (halvingChecked) {
            ...
            if (block.timestamp >= nextHalvingTime) {
                massUpdatePools();
                uint256 _rewardPerSecond = (rewardPerSecond * rewardHalvingRate) / 10000;
    ...
    function _harvestReward(uint256 _pid, address _account) internal {
        ...
        if (_amount > 0) {
            ...
            if (_claimableAmount > 0) {
                require(whitelistedContract[_account] || _claimableAmount * 100 <= IERC20(nil).totalSupply(), "Suspicious big reward amount!!");
                // <= 1% total supply
